...
/**
 * Writes `data` into the commit's artifact directory and records it. It takes bytes,
 * not a path: renderer output is read with readOutput() first (outputs.ts).
 */
export async function storeArtifact(opts: {
	...
	const target = path.join(dir, opts.targetName);
	await fsp.writeFile(target, opts.data);
	...
	run(
		...
		opts.kind,
		opts.name,
		path.join(opts.commitId, opts.targetName),
		opts.ordinal ?? 0,
...
export function svgGeometry(svg: Uint8Array) {
	const head = Buffer.from(svg.subarray(0, 4096)).toString('utf8');
