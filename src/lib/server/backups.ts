...
export async function createSnapshot(opts: { includeArtifacts: boolean; actorId: string }) {
	...
	try {
		...
		const dbCopy = path.join(work, 'pcbhub.db');
		...
		const manifest: SnapshotManifest = {
			...
			site_name: getSetting('site_name', 'PCBHub'),
			counts: {
		...
		const name = uniqueName(`pcbhub-snapshot-${stamp}${opts.includeArtifacts ? '' : '-lite'}.tar.gz`);
		const target = path.join(BACKUP_DIR, name);
		const partial = `${target}.partial`;

		// Manifest first so reading it back only has to decompress the archive's head.
		const args = ['-czf', partial, '-C', work, 'manifest.json', 'pcbhub.db', '-C', DATA_DIR, 'repos'];
