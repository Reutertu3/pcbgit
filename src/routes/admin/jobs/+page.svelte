...
<svelte:head><title>Render queue · Admin · {data.site.name}</title></svelte:head>

<div class="mb-4 flex flex-wrap items-center justify-between gap-2">
	<h2 class="text-lg font-semibold tracking-tight">Render queue</h2>
	<div class="flex flex-wrap gap-2">
		...
		<form method="POST" action="?/unstick" use:enhance>
			<button class="btn btn-sm" type="submit" title="Reset jobs stuck running for over 15 minutes">
				<Icon name="alert" size={13} /> Unstick
			</button>
		</form>
		<form method="POST" action="?/retryAllFailed" use:enhance>
			<button class="btn btn-sm" type="submit"><Icon name="refresh" size={13} /> Retry failed</button>
		</form>
		<form method="POST" action="?/clearFinished" use:enhance>
			<button class="btn btn-sm" type="submit"><Icon name="trash" size={13} /> Clear finished</button>
		</form>
	</div>
</div>
...
<div class="surface overflow-x-auto">
	<table class="w-full text-left text-[0.8125rem]">
		<thead class="bg-s2 text-xs">
			<tr>
				...
				<th class="px-3 py-2 font-semibold">Queued</th>
			...
			{#each data.jobs as job (job.id)}
				<tr class="border-t">
					...
					<td class="px-3 py-2">
						...
						{#if job.attempts > 1}
							<span class="text-[0.6875rem] text-[var(--text-muted)]">· try {job.attempts}</span>
						{/if}
					</td>
					...
					<td class="px-3 py-2">
						<div class="flex justify-end gap-1">
							<a href={logHref(job.id)} class="btn btn-sm" title="Render log"><Icon name="file" size={12} /></a>
							<form method="POST" action="?/retry" use:enhance>
								<input type="hidden" name="id" value={job.id} />
								<button class="btn btn-sm" type="submit" title="Re-queue"><Icon name="refresh" size={12} /></button>
							</form>
						</div>
					</td>
				</tr>
	...
	{#if !data.jobs.length}
		<p class="px-4 py-10 text-center text-sm text-[var(--text-muted)]">No jobs to show.</p>
	{/if}
</div>
