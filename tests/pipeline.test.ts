...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pcbhub-test-'));
process.env.PCBHUB_DATA_DIR = dataDir;
// Keep the test instance from creating the default admin with a weak password.
process.env.PCBHUB_ADMIN_PASSWORD = 'test-password-not-used';
